...
public class SpectatorEvents implements Listener {
	...
	public void onPlayerClickEvent(PlayerInteractEvent event) {
		Player player = event.getPlayer();
		try{
				if(GameManager.getInstance().isSpectator(player) && (event.getAction() == Action.RIGHT_CLICK_BLOCK)||
						GameManager.getInstance().isSpectator(player) && (event.getAction() == Action.LEFT_CLICK_BLOCK)||
						GameManager.getInstance().isSpectator(player) && (event.getAction() == Action.LEFT_CLICK_AIR)||
						GameManager.getInstance().isSpectator(player) && (event.getAction() == Action.RIGHT_CLICK_AIR)){
		...
		@EventHandler(priority = EventPriority.HIGHEST)
		public void onEntityPush(EntityInteractEvent event) {
			Player player = null;
			if (event.getEntity() instanceof Player) {
				player = (Player)event.getEntity();
			}
